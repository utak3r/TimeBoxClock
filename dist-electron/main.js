...
import electron, { ipcMain as ipcMain$1, Menu, app as app$1, BrowserWindow, nativeImage, Tray } from "electron";
...
function consolidateJobs(jobs, projects2, now = /* @__PURE__ */ new Date()) {
  const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
  const jobsToKeep = [];
  const jobsToConsolidate = [];
  jobs.forEach((job) => {
    if (job.isRunning) {
      jobsToKeep.push(job);
      return;
  ...
  for (const [key, group] of groups) {
    ...
    const endTime = Math.max(...group.map((j) => j.endTime || j.startTime + j.duration * 1e3));
...
const consoleLogger = {
  ...
  output(type2, args) {
    var _a, _b;
    (_b = (_a = console == null ? void 0 : console[type2]) == null ? void 0 : _a.apply) == null ? void 0 : _b.call(_a, console, args);
  }
};
class Logger {
...
class Translator extends EventEmitter {
  ...
  getResource(code2, ns, key, options = {}) {
    var _a;
    if ((_a = this.i18nFormat) == null ? void 0 : _a.getResource) return this.i18nFormat.getResource(code2, ns, key, options);
...
class Interpolator {
  ...
  resetRegExp() {
    const getOrResetRegExp = (existingRegExp, pattern2) => {
      if ((existingRegExp == null ? void 0 : existingRegExp.source) === pattern2) {
...
const initI18n = () => {
  ...
  if (!lng) {
    const sysLocale = app$1.getLocale() || "en";
...
ipcMain$1.on("log", (_, msg) => console.log("Renderer/Preload:", msg));
ipcMain$1.on("language-changed", (_, lang) => {
  console.log("Language changed to:", lang);
  store.set("language", lang);
  instance.changeLanguage(lang);
  updateTrayMenu();
});
ipcMain$1.handle("db:get-projects", () => {
  console.log("IPC: db:get-projects");
  return store.get("projects");
});
ipcMain$1.handle("db:add-project", (_, project) => {
  console.log("IPC: db:add-project", project);
  const projects2 = store.get("projects");
  store.set("projects", [...projects2, project]);
  return project;
});
ipcMain$1.handle("db:delete-project", (_, id2) => {
  console.log("IPC: db:delete-project", id2);
  const projects2 = store.get("projects");
  store.set("projects", projects2.filter((p) => p.id !== id2));
});
ipcMain$1.handle("db:update-project", (_, updatedProject) => {
  console.log("IPC: db:update-project", updatedProject);
  const projects2 = store.get("projects");
  store.set("projects", projects2.map((p) => p.id === updatedProject.id ? updatedProject : p));
  return updatedProject;
});
ipcMain$1.handle("db:get-jobs", () => {
  console.log("IPC: db:get-jobs");
  return store.get("jobs");
});
ipcMain$1.handle("db:add-job", (_, job) => {
  console.log("IPC: db:add-job", job);
  const jobs = store.get("jobs");
  store.set("jobs", [...jobs, job]);
  return job;
});
ipcMain$1.handle("db:update-job", (_, updatedJob) => {
  console.log("IPC: db:update-job", updatedJob);
  const jobs = store.get("jobs");
  store.set("jobs", jobs.map((j) => j.id === updatedJob.id ? updatedJob : j));
  return updatedJob;
});
ipcMain$1.handle("db:delete-job", (_, id2) => {
  console.log("IPC: db:delete-job", id2);
  const jobs = store.get("jobs");
  store.set("jobs", jobs.filter((j) => j.id !== id2));
});
...
function updateTrayMenu() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: instance.t("tray.showApp"), click: () => showWindow() },
    { type: "separator" },
    {
      label: instance.t("tray.quit"),
      click: () => {
        isQuitting = true;
        app$1.quit();
      }
